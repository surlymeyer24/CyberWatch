...
| ------------------------ | ----------------- | -------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **CyberWatch.Service**   | Session 0         | SYSTEM         | Detección de amenazas (puntuación + **refuerzo opcional por entropía** Shannon), respuesta activa (kill + cuarentena), sync Firestore, **monitor de puertos TCP**, **firma Authenticode en servicios Windows** (opcional), **política remota `config/servicios`** (hashes / exclusiones SCM; opcional), geolocalización IP, servidor Named Pipe, **anti-tampering**: vigilancia del UserAgent vía pipe + recuperación SCM (`sc failure`) |
| **CyberWatch.UserAgent** | Sesión de usuario | Usuario normal | GPS (Windows Location API), capturas de pantalla, historial de navegación, cliente Named Pipe, comandos remotos, **anti-tampering**: intento best-effort de arrancar el servicio Windows si el pipe cae de forma prolongada                                                                                                                                                                                                                                 |
...
### Auto-Protección (Anti-Tampering)

- **Capa 1 — SCM:** el instalador configura `sc failure` para que Windows reinicie el servicio **CyberWatch** si el proceso termina de forma inesperada (intervalos de 5 segundos entre reintentos).
- **Capa 2 — Watchdog mutuo (Named Pipe):** el Service envía `ping` cada ~20 s; si **no** hay ningún cliente conectado al pipe durante **más de 60 s**, fuerza `schtasks /Run` sobre la tarea `CyberWatch\UserAgent`. El UserAgent, si el pipe falla de forma continuada **más de 30 s**, intenta `ServiceController.Start("CyberWatch")` (suele fallar sin privilegios elevados; sirve como refuerzo donde aplique).
- **Apagado seguro:** durante **`actualizar_agente`** y **`reiniciar_servicio`** se crea el archivo `C:\ProgramData\CyberWatch\cyberwatch.updating`; los watchdogs lo respetan y los scripts `.bat` remotos lo borran al terminar. Detalle: [docs/iteracion-8-pasos.md](docs/iteracion-8-pasos.md).
- **Fuera de alcance en esta versión:** protección kernel/DACL contra `Process.Kill` (estilo EDR comercial).
...
| `cyberwatch_instancias/{machineId}/alertas/`              | Subcollection de alertas por máquina (ransomware, eventos de seguridad, **puertos TCP**, servicios no base, **firma de servicio** `servicio_sin_firma_valida`, **servicio anómalo** `servicio_no_firmado` con política remota). Dedup: no se crea alerta duplicada si ya existe una con los mismos campos clave en la ventana configurada. Ransomware: `extensionDetectada`, opcional entropía; puertos: `puertoLocal`/`pidProceso`; firma iter.6: `subjectFirma`, `razonFirma`; iter.7: `hashEjecutableSha256` |
...
| `config/servicios`                                        | Política remota para **servicios anómalos** (listener): `nombres_excluidos` (SCM), `hashes_permitidos` (SHA-256 hex del PE), `ultima_modificacion`. Ejemplo: [docs/config-servicios.example.json](docs/config-servicios.example.json) · [docs/iteracion-7-pasos.md](docs/iteracion-7-pasos.md)                                                                                                                                                                                                                  |
...
> **Índice compuesto (alertas de firma de servicio y servicio anómalo):** deduplicación con `tipo` + `nombreServicio` + `fechaHora`; mismo archivo `firestore.indexes.json` (comparte el índice de la iteración 6).
...
| -------------------- | --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `actualizar_agente`  | Service   | Descarga el ZIP del release, extrae, lanza batch de actualización (`Process.Start` directo, sin Task Scheduler), reinicia servicio y UserAgent. Crea el lock `cyberwatch.updating` para que los watchdogs no interfieran; el batch lo elimina al finalizar. Post-reinicio: fuerza creación de tarea en Task Scheduler, ejecuta UserAgent, reporta log del batch en `comando_resultado`                                                                             |
| `reiniciar_servicio` | Service   | Lanza batch que hace `net stop` + `net start` del servicio. Usa el mismo lock `cyberwatch.updating` durante el reinicio programado                                                                                                                                                                                                                         |
...
- **Watchdog:** si el servidor permanece sin clientes más de 60 s (y no hay lock `.updating`), relanza el UserAgent vía Task Scheduler; si el cliente lleva más de 30 s sin pipe estable (y no hay lock), intenta iniciar el servicio Windows
...
3. Registra/actualiza el Windows Service (`sc create`/`sc config`), configura **`sc failure`** (reinicio automático del servicio si el proceso muere) y ejecuta `net start`
